...
func TestRSAVerify(t *testing.T) {
	keyData, _ := os.ReadFile("../00_files/sample_key.pub")
	key, _ := pemc.ToRSAPublicKey(keyData)
	...
	for _, data := range rsaTestData {
		...
		decodedSignature := decodeSegment(t, parts[2])
		err = method.Verify(strings.Join(parts[0:2], "."), decodedSignature, key)
...
func TestRSASign(t *testing.T) {
	keyData, _ := os.ReadFile("../00_files/sample_key")
	key, _ := pemc.ToRSAPrivateKey(keyData)
...
func TestRSAVerifyWithPreParsedPrivateKey(t *testing.T) {
	...
	parsedKey, err := pemc.ToRSAPublicKey(key)
	if err != nil {
		t.Fatal(err)
	}
	testData := rsaTestData[0]
	parts := strings.Split(testData.tokenString, ".")
	err = alg.RS256.Verify(strings.Join(parts[0:2], "."), decodeSegment(t, parts[2]), parsedKey)
	if err != nil {
...
func TestRSAWithPreParsedPrivateKey(t *testing.T) {
	...
	parsedKey, err := pemc.ToRSAPrivateKey(key)
	if err != nil {
		t.Fatal(err)
	}
	testData := rsaTestData[0]
	parts := strings.Split(testData.tokenString, ".")
	sig, err := alg.RS256.Sign(strings.Join(parts[0:2], "."), parsedKey)
	if err != nil {
...
func TestRSAKeyParsing(t *testing.T) {
	key, _ := os.ReadFile("../00_files/sample_key")
	secureKey, _ := os.ReadFile("../00_files/privateSecure.pem")
	pubKey, _ := os.ReadFile("../00_files/sample_key.pub")
	...
	if _, e := pemc.ToRSAPrivateKey(key); e != nil {
		t.Errorf("Failed to parse valid private key: %v", e)
	}

	if k, e := pemc.ToRSAPrivateKey(pubKey); e == nil {
		t.Errorf("Parsed public key as valid private key: %v", k)
	}

	if k, e := pemc.ToRSAPrivateKey(badKey); e == nil {
		t.Errorf("Parsed invalid key as valid private key: %v", k)
	}

	if _, e := pemc.ToRSAPrivateKeyWithPassword(secureKey, "password"); e != nil {
		t.Errorf("Failed to parse valid private key with password: %v", e)
	}

	if k, e := pemc.ToRSAPrivateKeyWithPassword(secureKey, "123132"); e == nil {
		t.Errorf("Parsed private key with invalid password %v", k)
	}

	// Test parsePublicKey
	if _, e := pemc.ToRSAPublicKey(pubKey); e != nil {
		t.Errorf("Failed to parse valid public key: %v", e)
	}

	if k, e := pemc.ToRSAPublicKey(key); e == nil {
		t.Errorf("Parsed private key as valid public key: %v", k)
	}

	if k, e := pemc.ToRSAPublicKey(badKey); e == nil {
		t.Errorf("Parsed invalid key as valid private key: %v", k)
	}

	if _, err := pemc.ToRSAPublicKey(pkcs1Buffer.Bytes()); err != nil {
...
func BenchmarkRSAParsing(b *testing.B) {
	...
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			if _, err := pemc.ToRSAPrivateKey(key); err != nil {
...
// 	key, _ := os.ReadFile("../00_files/sample_key")
// 	parsedKey, err := alg.ParseRSAPrivateKeyFromPEM(key)
// 	if err != nil {
// 		b.Fatal(err)
// 	}

// 	benchmarkSigning(b, alg.RS256, parsedKey)
// }
...
// 	benchmarkSigning(b, alg.RS384, parsedKey)
// }
...
// 	benchmarkSigning(b, alg.RS512, parsedKey)
// }
